# Changelog

All notable project-record changes are tracked here.

## 2026-04-26

### Added

- Added `STATUS.md` as the active truth file for validation state and open unknowns.
- Added `HANDOFF.md` for compact restart context.
- Added `TASKS.md` for the current work queue.
- Added `DECISIONS.md` for project decisions.
- Added `PROJECT_RECORDS.md` for Human-first 2026 documentation style.

### Changed

- Rewrote `README.md` from a long legacy reference into a Human-first project entrypoint.
- Moved compatibility uncertainty into explicit status language.
- Reframed hotkeys as a quick scanning table.
- Removed stale donation-first framing from the top of the README.

### Not Changed

- No AutoHotkey code was changed.
- No DLL was changed.
- No compatibility test was performed in this pass.
- No release package was created.
